...
function getNeighbours(_grid, curNodePos)
{
	let list = new Array();
	if (curNodePos.x - 1 >= 0 && (_grid.array[curNodePos.y][curNodePos.x - 1].value == 0 || _grid.array[curNodePos.y][curNodePos.x - 1].value == 3))
		list.push(_grid.array[curNodePos.y][curNodePos.x - 1]);
	if (curNodePos.x + 1 < amount && (_grid.array[curNodePos.y][curNodePos.x + 1].value == 0 || _grid.array[curNodePos.y][curNodePos.x + 1].value == 3))
		list.push(_grid.array[curNodePos.y][curNodePos.x + 1]);
	if (curNodePos.y - 1 >= 0 && (_grid.array[curNodePos.y - 1][curNodePos.x].value == 0 || _grid.array[curNodePos.y - 1][curNodePos.x].value == 3))
		list.push(_grid.array[curNodePos.y - 1][curNodePos.x]);
	if (curNodePos.y + 1 < amount && (_grid.array[curNodePos.y + 1][curNodePos.x].value == 0 || _grid.array[curNodePos.y + 1][curNodePos.x].value == 3))
		list.push(_grid.array[curNodePos.y + 1][curNodePos.x]);
...
{
	return floor(dist(startNode.pos.x, startNode.pos.y, endNode.pos.x, endNode.pos.y));
}
...
function search(_path, g, bound, destNode, _grid)
{
	...
	if (cur_node.value == 0) // a supprimer
		cur_node.value = 3;
	let d = getDistance(cur_node, destNode);
	let f = g + d;
	if (f > bound)
	{
		cur_node.value = 0;
...
/*
function search(_grid, curNode, destNode, distance, bound, _path)
{
	if (isGoal(curNode, destNode))
		return -distance;
	let estimate = distance + getDistance(curNode, destNode);
	if (estimate > bound)
		return estimate;
	let min = Infinity;
	let neighbours = getNeighbours(_grid, curNode.pos);
	for(let i = 0; i < neighbours.length; ++i)
	{
		_path.push(neighbours[i]);
		let t = search(_grid, neighbours[i], destNode, distance + getDistance(curNode, neighbours[i]), bound, _path);	
		if (t < 0)
			return t;
		else if (t < min)
			min = t;
...
{
	let _path = new Array();
	let curNode = _grid.array[startPos.y][startPos.x];
	let destNode = _grid.array[endPos.y][endPos.x];
	let bound = getDistance(curNode, destNode);
	while (Infinity)
	{
		let distance = search(_grid, curNode, destNode, 0, bound, _path);
